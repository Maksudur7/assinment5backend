...
async function main() {
    ...
    for (const category of categories) {
        await prisma_1.default.category.upsert({ where: { name: category.name }, update: { icon: category.icon }, create: category });
    }
    const mediaItems = [
        {
    ...
    for (const item of mediaItems) {
        await prisma_1.default.media.upsert({ where: { streamingUrl: item.streamingUrl }, update: item, create: item });
    }
}
main()
    .catch((error) => {
