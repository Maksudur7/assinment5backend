...
import { getWatchlist, removeFromWatchlist, toggleWatchlist, getHistory, addToHistory } from "./watchlist.service";
...
export async function getHistoryController(req: Request, res: Response) {
	if (!req.user) throw new AppError("Unauthorized", 401, "UNAUTHORIZED");
	return res.status(200).json(await getHistory(req.user.id));
}

export async function addToHistoryController(req: Request, res: Response) {
	if (!req.user) throw new AppError("Unauthorized", 401, "UNAUTHORIZED");
	return res.status(200).json(await addToHistory(req.user.id, String(req.params.mediaId)));
}
