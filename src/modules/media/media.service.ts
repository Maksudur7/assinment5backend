...
export async function listMedia(query: Record<string, unknown>) {
	...
	const orderBy = sort === "latest" ? { createdAt: "desc" as const } : { popularity: "desc" as const };
...
export async function listRecommendations(userId: string) {
	const watchlist = await prisma.watchlistItem.findMany({ where: { userId }, include: { media: true }, take: 20 });
...
export async function updateMedia(id: string, payload: Record<string, unknown>) {
	const media = await prisma.media.findUnique({ where: { id } });
	if (!media) throw new AppError("Media not found", 404, "MEDIA_NOT_FOUND");
